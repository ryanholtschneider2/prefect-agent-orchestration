...
| `beads_meta.py` | `MetadataStore` Protocol; `BeadsStore` (shells `bd`) + `FileStore` (JSON fallback); `claim_issue`/`close_issue`/`list_epic_children`. `_bd_dep_list`/`_bd_show` resolve the backend binary (`bd`/`br`) and normalize br dep rows via `beads_backend`. `iter_bead_id(seed, step, iter_n)` / `iter_bead_re(seed)` are the single source of truth for the br-safe `<seed>-<step>-iter<N>` iter-bead convention — every iter-id construction in core + packs routes through them. |
