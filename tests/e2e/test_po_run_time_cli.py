"""E2E argv-shape regressions for `po run --time` (issue prefect-orchestration-7jr).
...
These are the things this layer alone can catch:
  * the `--time` flag is wired into `--help`
  * `po run nonexistent --time 2h` exits non-zero cleanly without a
    Python traceback (regression for AC §3 — adding `--time` did not
    break the unknown-formula path)
"""
...
def test_po_run_help_advertises_time_flag() -> None:
    result = _po("run", "--help")
...
def test_po_run_unknown_formula_with_time_fails_cleanly() -> None:
    """Adding --time must not crash the unknown-formula path."""
...
def test_po_run_garbage_time_fails_with_clean_error() -> None:
    """Bad --time spec is caught before any Prefect call. We need a
    ...
    result = _po("run", "nonexistent-formula", "--time", "yesterday")
    assert result.returncode != 0
    assert "Traceback" not in result.stderr
    assert "Traceback" not in result.stdout
