...
function IssueDetail({issue, width, liveOutput, liveScroll, followOutput}: {issue: Issue; width: number; liveOutput?: string; liveScroll: number; followOutput: boolean}) {
  ...
  return <Box flexDirection="column">
    ...
    <Text> </Text><Label>Live agent output {followOutput ? "[follow]" : `[scroll ${liveScroll}]`}</Label>{visibleOutput.length ? visibleOutput.map((line, index) => <Text key={index} dimColor>{truncateCells(line, width)}</Text>) : <Text dimColor>No active agent session.</Text>}
