...
export function ActionForm({form, selection, colors}: {form: ActionFormView; selection?: Selection; colors: Theme}) {
  ...
  const choices = spec?.key === "artifactPath" ? (selection?.kind === "issue" ? selection.artifacts : selection?.kind === "epic" ? selection.children.flatMap((issue) => issue.artifacts) : []).map((item) => item.path) : spec?.key === "sessionTarget" && selection?.kind === "issue" ? selection.sessions.filter((item) => item.available).map((item) => item.target) : [];
  return <Box borderStyle="single" borderColor={colors.accent} paddingX={1} flexDirection="column">
    <Text bold>{form.action.title}</Text><Text dimColor>target: {selection?.id ?? "global"}</Text><Text> </Text>
    {form.stage === "args" && spec ? <><Text>{spec.label} {spec.required ? "(required)" : ""}</Text>{choices.map((choice, index) => <Text key={choice} color={index === 0 ? colors.accent : undefined}>{index === 0 ? ">" : " "} {choice}</Text>)}{choices.length === 0 && (spec.key === "artifactPath" || spec.key === "sessionTarget") ? <Text color={colors.warning}>No discovered choices; this action cannot execute.</Text> : null}<Text color={colors.accent}>value: {form.input}</Text><Text dimColor>Enter accepts · Esc default-cancels the command</Text></> : null}
