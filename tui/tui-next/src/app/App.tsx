...
import {ActionCoordinator, filterActions, type OperatorAction, type Selection} from "../actions/registry.js";
...
export function App({rigPath, prefectUrl, refreshMs, ascii = false, initialModel, onAttach, dimensions}: AppProps) {
  ...
  const actionCoordinator = useMemo(() => new ActionCoordinator(undefined, async (action, object, args) => {
    ...
    if (action.id === "state") { const snapshot = await fetchBeads(rigPath); const row = snapshot.data.find((item) => item.id === object.id); return snapshot.freshness === "fresh" ? row?.status === args.state : undefined; }
    if (["dispatch", "retry", "pause", "resume", "cancel"].includes(action.id)) {
      ...
      if (action.id === "dispatch" || action.id === "retry") return attempts.length > 0 || undefined;
  ...
  useEffect(() => {
    ...
    const target = selected?.kind === "issue" && selected.attempts[0]?.roles.at(-1) ? `po-${selected.id}-${selected.attempts[0].roles.at(-1)!.role}` : undefined;
    ...
    controllers.current.push(controller as SourceController<unknown>); const unsub = controller.subscribe((snapshot) => publish("tmux", snapshot as SourceSnapshot<unknown>)); controller.start();
  ...
  }, [initialModel, publish, refreshMs, selected?.id]);
